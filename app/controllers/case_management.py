...
def case_upload_machine(case_id):
    ...
    # upload the machine files ajax
        ...
        print("-----FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFile HERE ----")
        print(request.files)
        sys.stdout.flush()
        # get file
        file = request.files['files[]']
        # if there is a file to upload
        print("-----FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFile Sent ----")
        sys.stdout.flush()
        if file:
...
def timeline_data():
    if request.method == "POST":
        ...
        url = 'http://127.0.0.1:5002/timeline/data/'
        data_to_send = {'data': request.json['data']}
...
# --------------------------------------------------- Custom Upload machines automatically from supporting app
# upload machine page to upload files to the artifacts file upload,
# this is the page allow to upload multiple zip file, each considered as machine
@app.route("/custom/case/<case_id>/upload_machine" , methods=['POST' , 'GET'])
def custom_case_upload_machine(case_id):
    if request.method == 'POST':
        # get file
        body = request.form
        print(body)
